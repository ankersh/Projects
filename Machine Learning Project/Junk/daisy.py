...
for i in range(0, 10):
    ...
    timg = cv2.adaptiveThreshold(rawimage,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY,21,2)
    rawimage = timg.flatten()
...
Xtrain, Xtest, Ytrain, Ytest = train_test_split(trainimages, labels, test_size=0.5,random_state=10)          #Splits training data randomly
...
result.to_csv("DAISYClassPredictions.csv")
